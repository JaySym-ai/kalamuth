-- Fix infinite recursion in combat_match_acceptances RLS policies
-- The issue is caused by complex joins in the select policy that create circular dependencies
...
-- Simplified select policy - users can see acceptances for matches they participate in
-- This avoids the complex join that was causing recursion
...
-- Allow users to insert their own acceptances (for accept/decline actions)
create policy "Users can insert own acceptances"
  on public.combat_match_acceptances
  for insert
  with check ("userId" = auth.uid());

-- Allow users to update their own acceptances (for accept/decline actions)
