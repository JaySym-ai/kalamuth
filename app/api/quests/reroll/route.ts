...
export async function POST(req: Request) {
  try {
    ...
    if (treasury.amount < REROLL_COST) {
      return NextResponse.json(
        { error: "insufficient_funds", required: REROLL_COST, available: treasury.amount },
        { status: 400 }
      );
    }
    ...
    const newAmount = treasury.amount - REROLL_COST;
    ...
    if (deleteErr) {
      return NextResponse.json({ error: "failed_to_delete_quest" }, { status: 500 });
    }

    return NextResponse.json({
      success: true,
      questId,
      message: `Quest rerolled. ${REROLL_COST} sestertii deducted.`,
