...
function extractApiErrorDetails(error: unknown): ApiErrorDetails {
  ...
  return { status, code, type, message, raw: rawSource ? safeSerialize(rawSource) : null };
}
...
function extractProviderErrorDetails(payload: unknown): { message: string; code: string | number | null; raw: string | null } | null {
  ...
  return { message, code, raw: safeSerialize(providerError) };
}
...
export interface BilingualText { en: string; fr: string }
export interface BilingualGladiatorStats {
  strength: BilingualText; agility: BilingualText; dexterity: BilingualText; speed: BilingualText; chance: BilingualText; intelligence: BilingualText; charisma: BilingualText; loyalty: BilingualText;
}
export interface GeneratedGladiatorStats {
...
function expectBilingualText(value: unknown, field: string): BilingualText {
  if (!value || typeof value !== 'object') throw new Error(`Field "${field}" must be a bilingual object with 'en' and 'fr' keys`);
...
function expectIntegerInRange(value: unknown, field: string, min: number, max: number): number {
  ...
  if (!Number.isFinite(numeric)) throw new Error(`Field "${field}" must be an integer between ${min} and ${max}`);
  const rounded = Math.round(numeric);
  if (rounded < min || rounded > max) throw new Error(`Field "${field}" must be between ${min} and ${max}`);
  return rounded;
}
...
export async function generateOneGladiator(client: OpenAI, context?: GenerationContext): Promise<GeneratedGladiator | BilingualGeneratedGladiator> {
  ...
  for (let generationAttempt = 1; generationAttempt <= 3; generationAttempt++) {
    ...
    const choiceMessage = choice?.message as { role?: string; content?: string; tool_calls?: unknown[]; reasoning?: unknown; reasoning_details?: unknown } | undefined;
    ...
    const reasoningText = choiceMessage && typeof (choiceMessage as { reasoning?: unknown }).reasoning === 'string' ? (choiceMessage as { reasoning: string }).reasoning : null;
    ...
    try {
      ...
      const lifeGoalCandidate = typeof parsedObj.lifeGoal === 'object' && parsedObj.lifeGoal !== null ? (parsedObj.lifeGoal as Record<string, unknown>) : null;
      const isBilingual = typeof lifeGoalCandidate?.['en'] === 'string' && typeof lifeGoalCandidate?.['fr'] === 'string';
